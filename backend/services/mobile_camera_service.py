...
def probe_source(source_url: str, timeout_seconds: float = 5.0) -> dict:
    ...
    try:
        ...
        if not cap.isOpened():
            write_log("mobile", "ERROR", f"mobile camera connection refused: cannot open source url={redact_source_url(source_url)}")
            raise ValueError("无法打开手机视频源")
        ok, frame = cap.read()
        if not ok or frame is None:
            write_log("mobile", "ERROR", f"mobile camera stream unreachable: no frame returned url={redact_source_url(source_url)}")
        ...
        write_log("mobile", "ERROR", f"mobile camera network error: {type(exc).__name__} {exc} url={redact_source_url(source_url)}")
