...
- 手机摄像头：车主端可选择“手机摄像头”，测试并连接 RTSP、RTMP 或 HTTP(S) 视频流，再将最新画面接入优化交警手势识别。

## 手机摄像头接入

推荐在答辩电脑上运行 MediaMTX，并让手机和电脑处于同一局域网。手机推送地址示例为
`rtmp://<电脑局域网IP>:1935/live/phone`，IRV 中填写读取地址
`rtsp://127.0.0.1:8554/live/phone`。也支持手机摄像头应用直接提供的 HTTP 视频地址。

1. 在车主端“车外视角”选择“手机摄像头”。
2. 输入视频流地址，先点击“测试连接”；成功后点击“连接手机”。
3. 点击“开始交警识别”，系统只推理采集到的最新帧，避免排队造成画面延迟。
4. 停止推流或结束演示时点击“断开”。接口状态可通过 `GET /api/mobile-camera/status` 查看。

视频地址中的用户名和密码不会写入状态响应或日志。若“测试连接”超时，请先用 VLC 验证流地址、局域网连通性和 Windows 防火墙。
